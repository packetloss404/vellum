...
def bulk_replace_plan_items_with_conn(
    ...
    existing_sub_ids: dict[str, Optional[str]] = {
        row["id"]: row["sub_investigation_id"]
        for row in conn.execute(
            "SELECT id, sub_investigation_id FROM plan_items WHERE dossier_id = ?",
            (dossier_id,),
        ).fetchall()
        if row["sub_investigation_id"] is not None
    }
    conn.execute(
        "DELETE FROM plan_items WHERE dossier_id = ?", (dossier_id,)
    )
    ...
    for item_id, sub_id in existing_sub_ids.items():
        conn.execute(
            "UPDATE plan_items SET sub_investigation_id = ? WHERE id = ? AND dossier_id = ?",
            (sub_id, item_id, dossier_id),
        )
    rows = conn.execute(
        "SELECT * FROM plan_items WHERE dossier_id = ? ORDER BY order_key",
        (dossier_id,),
