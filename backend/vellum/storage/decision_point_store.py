...
def resolve_decision_point(
    ...
    with connect() as conn:
        ...
        if not row:
            return None
        cur = conn.execute(
            "UPDATE decision_points SET resolved_at = ?, chosen = ? WHERE id = ? AND resolved_at IS NULL",
        ...
        if (
            ...
            if plan_row and plan_row["investigation_plan"]:
                current_plan = m.InvestigationPlan.model_validate_json(plan_row["investigation_plan"])
                if current_plan.approved_at is None:
                    approved_plan = current_plan.model_copy(update={"approved_at": m.utc_now()})
                    conn.execute(
